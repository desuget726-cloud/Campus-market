...
#     For now this returns a static list so the frontend can fetch it dynamically.
#     In future this can be moved to a dedicated table and queried from the DB.
#     """
#     # Returning the same canonical campus list used earlier in the frontend.
#     return [
...
def _parse_price(value: Optional[str]) -> float:
    if value is None:
        return 0.0
...
@app.delete("/api/student/wishlist/{id}")
...
def add_to_cart(data: CartItemCreate, db: Session = Depends(get_db)):
    ...
    cart_item = (
        db.query(CartItem)
        .filter(CartItem.student_id == data.student_id, CartItem.product_id == data.product_id)
        .first()
    )
    ...
    new_item = CartItem(student_id=data.student_id, product_id=data.product_id, quantity=1)
    db.add(new_item)
    db.commit()
    db.refresh(new_item)
...
# 3.6 የቤት ገጽ ማስታወቂያ መረጃ ማውጫ (GET /api/student/highlights)
...
def get_student_highlights(student_id: str, db: Session = Depends(get_db)):
    ...
    for item in cart_items:
        ...
        if product:
            try:
                price_value = float(str(product.price).replace('$', '').replace('ETB', '').replace(',', '').strip())
            except (ValueError, TypeError):
                price_value = 0.0
            cart_value += price_value * (item.quantity or 1)

    latest_listings = db.query(Product).filter(Product.status == "Approved").count()

    return {
        "aiPicks": 5,
        "latestListings": latest_listings,
        "cartValue": round(cart_value, 2),
        "pendingMessages": unread_messages
    }

# 3.7 ለአስተዳዳሪው የኮከብ ደረጃ እና አስተያየት መላኪያ (POST /api/student/reviews) — አሁን ከዳታቤዝ ያነባል
...
# የቻፓ የሙከራ ሚስጥር ቁልፍ (ከቻፓ ዳሽቦርድዎ ላይ ወስደው እዚህ ይተኩት)
CHAPA_SECRET_KEY = os.getenv("CHAPA_SECRET_KEY", "CHASECK_TEST-xxxxxxxxxxxxxxxx")

# 1. የክፍያ ጥያቄ አስጀምሮ የመክፈያ ሊንክ ለሪአክት የሚልክ ኤፒአይ (POST /api/payment/initialize)
@app.post("/api/payment/initialize")
async def initialize_chapa_payment(data: DepositRequest, db: Session = Depends(get_db)):
    student = db.query(Student).filter(Student.student_id == data.student_id).first()
    if not student:
        raise HTTPException(status_code=404, detail="Student not found")

    # ልዩ የትራንዛክሽን ቁጥር መፍጠር (e.g. TX-DEP-1A2B3C)
    tx_ref = f"TX-DEP-{uuid.uuid4().hex[:6].upper()}"

    # ክፍያው እስኪጠናቀቅ በ "Pending" ሁኔታ በዳታቤዝ ውስጥ ይመዘገባል
    db_tx = Transaction(
        student_id=data.student_id,
        tx_id=tx_ref,
        type="Deposit",
    ...
    chapa_email = data.email or student.email
    if not chapa_email:
        raise HTTPException(status_code=400, detail="Email is required for payment initialization.")

    # ለቻፓ ሰርቨር የሚላክ መረጃ (Chapa Payload)
    chapa_payload = {
        ...
        "tx_ref": tx_ref,
        "callback_url": "http://127.0.0.1:8000/api/payment/webhook",
        "return_url": "http://localhost:5173/#buyer",
        "customization": {
            "title": "Campace Wallet Deposit",
            "description": "Funding student digital wallet"
        }
    }
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code < 300 and chapa_data.get("status") == "success" and checkout_url:
                return {"checkout_url": checkout_url}
            raise HTTPException(status_code=400, detail=chapa_data.get("message", "Chapa initialization failed"))
        except httpx.HTTPError as e:
            raise HTTPException(status_code=500, detail=f"Payment gateway connection failed: {str(e)}")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Payment gateway error: {str(e)}")


# 2. ቻፓ ክፍያው መጠናቀቁን ለሰርቨራችን የሚያሳውቅበት የደህንነት ኤፒአይ (POST /api/payment/webhook)
@app.post("/api/payment/webhook")
async def chapa_webhook(payload: dict, db: Session = Depends(get_db)):
    tx_ref = payload.get("tx_ref")
    status = payload.get("status")

    if status == "success" and tx_ref:
        # በዳታቤዝ ውስጥ "Pending" ሆኖ የቆየውን ክፍያ ይፈልጋል
        transaction = db.query(Transaction).filter(Transaction.tx_id == tx_ref, Transaction.status == "Pending").first()
        if transaction:
            # የትራንዛክሽኑን ሁኔታ ወደ ስኬታማ ይቀይራል
            transaction.status = "Successful"
            
            # የተማሪውን የዋሌት ቀሪ ሂሳብ በ MySQL ውስጥ ያሳድጋል (Credit Wallet Balance)
            student = db.query(Student).filter(Student.student_id == transaction.student_id).first()
            if student:
                student.wallet_balance = float(student.wallet_balance) + float(transaction.amount)
            
            db.commit()
            return {"status": "success", "message": "Wallet balance credited successfully!"}
            
    return {"status": "ignored"}

# የይለፍ ቃል መቀየሪያ ፎርማቶች (Pydantic Schemas)
...
# ሀ. የይለፍ ቃል መቀየሪያ ባለ 6-አሃዝ ኮድ ማመንጫ (POST /api/auth/forgot-password)
@app.post("/api/auth/forgot-password")
def forgot_password(data: ForgotPasswordRequest, db: Session = Depends(get_db)):
    # ኢሜይሉ በተማሪዎች ወይም በአስተዳዳሪ ሰንጠረዥ ውስጥ መኖሩን ያረጋግጣል
    user = db.query(Student).filter(Student.email == data.email).first() or \
           db.query(Admin).filter(Admin.email == data.email).first()
    
    if not user:
        raise HTTPException(status_code=404, detail="Email address not found.")

    # ራንደም ባለ 6-አሃዝ ኮድ መፍጠር (e.g. 832049)
    otp = f"{random.randint(100000, 999999)}"
    expiry = datetime.utcnow() + timedelta(minutes=10) # ከ 10 ደቂቃ በኋላ ጊዜው ያልፋል
    ...
    return {"message": "Verification code has been successfully sent to your Gmail address!"}

# ለ. ኮዱን አረጋግጦ አዲሱን ፓስወርድ በዳታቤዝ የሚተካ (POST /api/auth/reset-password)
@app.post("/api/auth/reset-password")
def reset_password(data: ResetPasswordRequest, db: Session = Depends(get_db)):
    # ኮዱ ትክክለኛ፣ ያልተነበበ መሆኑን ያረጋግጣል
    ...
    # አዲሱን የይለፍ ቃል በ bcrypt ያመጥረዋል
    hashed_password = pwd_context.hash(data.new_password)

    # በተማሪዎች ወይም በአስተዳዳሪው ሰንጠረዥ ውስጥ ፈልጎ ፓስወርዱን ይቀይረዋል
