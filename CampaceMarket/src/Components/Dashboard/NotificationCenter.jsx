    ...
    return (
        <div className="mx-auto max-w-5xl px-4 py-6">
            <section className="min-h-[550px] rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                {groupedNotifications.length === 0 ? <div className="mt-8 rounded-[28px] border border-dashed border-slate-200 bg-slate-50 p-12 text-center"><div className="text-4xl">🔔</div><p className="mt-4 text-lg font-bold text-slate-900">You're all caught up! There are no new notifications right now.</p><p className="mt-2 text-sm text-slate-500">New campus activity will appear here as it happens.</p></div> : <div className="mt-8 space-y-8">{groupedNotifications.map((group) => <section key={group.label}><div className="mb-3 flex items-center gap-3"><h3 className="text-xs font-black tracking-[0.25em] text-slate-500">{group.label}</h3><span className="h-px flex-1 bg-slate-200" /></div><div className="space-y-3">{group.items.map((notification) => { const priority = getPriority(notification); const category = getCategory(notification); const notificationDate = formatNotificationDate(notification.created_at); return <article key={notification.id ?? `${notification.created_at}-${notification.title}`} className={`border-l-4 ${priority.border} rounded-r-2xl border-y border-r border-slate-200 bg-slate-50/70 p-5 transition-all duration-200 hover:bg-slate-100 hover:shadow-md`}><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className={`mt-1 h-3 w-3 shrink-0 rounded-full ${priority.color}`} title={priority.label} /><div><div className="flex flex-wrap items-center gap-2"><h4 className="font-black text-slate-950">{notification.title || 'Campus update'}</h4><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">{priority.icon} {priority.label}</span></div><p className="mt-2 text-sm leading-6 text-slate-600">{notification.message}</p></div></div><div className="shrink-0 text-left sm:text-right"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${notification.read ? 'bg-slate-200 text-slate-500' : 'bg-emerald-100 text-emerald-700'}`}>{notification.read ? 'Read' : 'New'}</span><p className="mt-2 text-xs text-slate-400">{notificationDate}</p></div></div><div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-200 pt-3"><span className="text-xs font-bold uppercase tracking-wider text-slate-400">{category}</span>{category !== 'System' && <button type="button" onClick={() => handleAction(notification)} className="text-sm font-black text-sky-700 hover:text-sky-900">{category === 'Orders' ? 'View Order →' : category === 'Messages' ? 'Reply →' : category === 'Products' ? 'View Product →' : 'View Payment →'}</button>}</div></article>; })}</div></section>)}</div>}
            </section>
        </div>
    );
}
