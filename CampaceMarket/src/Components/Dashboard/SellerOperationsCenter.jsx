...
function SellerOperationsCenter({ user, sellerData, sellerDashboardData, sellerOrdersLoading, sellerOrdersError, onRefreshOrders, myListings, setMyListings, setSellerData, setSellerDashboardData, onAddProduct: onCreateProduct, onNavigate: onTabNavigate, onViewProduct, onPaymentHistory, onEditProduct, onDeleteProduct, onTogglePause, onMarkAsSold, onApplyPriceDrop, onAdjustPrice, openOrderId, onOpenOrderHandled }) {
    ...
    const counts = {
        ...
        pending: Number(dashboardStats.pending_listings ?? calculatedCounts.pending),
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{[
                ...
                ['Received Orders', Number(dashboardStats.received_orders ?? orders.length), 'All received orders', '▤', 'text-emerald-600'],
                ['Total Revenue', formatSellerEtb(dashboardStats.completed_revenue), 'Completed orders', '◈', 'text-amber-600'],
                ['Pending Orders', Number(dashboardStats.pending_orders ?? pendingOrders.length), `${Number(dashboardAlerts.pending_orders ?? pendingOrders.length)} require action`, '◔', 'text-rose-600'],
